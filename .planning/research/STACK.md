...
**Critical design decision:** Do NOT route PDF uploads through Vercel serverless functions.

Vercel imposes a **4.5MB request body limit** on serverless functions. While bank statements are typically under 2MB, this is a brittle constraint. Instead:

1. **Upload:** Browser uploads PDF directly to Supabase Storage using the Supabase JS client
2. **Parse:** Server action (or API route) downloads the PDF from Storage using a signed URL, then parses with unpdf
3. **Store:** Parsed transactions go to the `transactions` table in Postgres

This architecture also means PDFs are retained for re-parsing when parser configs improve.
...
**PKCE flow** is the default (and required for SSR). Session tokens stored in cookies, not localStorage. Middleware must be configured to refresh tokens on every request.

### RLS Strategy

For a household app with admin/viewer roles:

```sql
-- Simple role-based access pattern
-- Store role in auth.users metadata or a profiles table
-- All data tables use a simple "authenticated users can read" policy
-- Write policies check for admin role
...
**Performance note:** Index the `profiles.id` column (it's the PK, so already indexed) and any columns used in RLS WHERE clauses. For a 2-3 user app, RLS performance is a non-issue, but the pattern should be correct from the start.
...
Alternative: make changes in Supabase Studio (local), then `supabase db diff --schema public` to auto-generate migration SQL.
...
**Do NOT use a money library.** This is a single-currency (SGD) app with no exchange rates, no multi-currency arithmetic, and no financial calculations beyond summing and grouping.
...
If multi-currency is ever needed (v2+), adopt Dinero.js then. Its v2 API is designed for this migration path.

## Installation

```bash
# Initialize Next.js with TypeScript, Tailwind, App Router
npx create-next-app@latest fin-genie --typescript --tailwind --eslint --app --src-dir
...
1. Visit [neobrutalism.dev/docs/installation](https://www.neobrutalism.dev/docs/installation)
2. Replace `globals.css` with their styling (CSS variables for thick borders, shadows, bold colors)
3. Add components via their registry: `npx shadcn add` with neobrutalism component URLs
4. Note: neobrutalism components only support CSS variables mode (not utility classes)

Alternatively, use the shadcn Neo Brutalism theme from the official registry at `shadcn.io/theme/neo-brutalism` and apply the CSS variables.
